...
const FilterBar: FC<FilterBarProps> = (props) => {
  ...
  const _getDom = async () => {
    ...
    if (topDomName) {
      const res1: any = await getDom(topDomName)
  ...
  return (
    <Fragment>
      ...
      <View id='filterbar' className={classnames('filterbar', className)}>
        ...
        <Etransition in={filterShow} timeout={300} animation='ele-top'>
          <View className={classnames('filterbar-filter')}>
            ...
            <View className='filter-item'>
              <View className='filter-item-title'>{atFilter.serve.title}</View>
